...
contract PointsHook is BaseHook, ERC1155 {
    ...
    function _afterSwap(
        ...
        // Check if the pair is ETH-Token pair, note: currency0 will always be the native toke (ETH)
        if(!key.currency0.isAddressZero()) return (this.afterSwap.selector, 0);

        // Check if user is buying Token with ETH not vice-versa
    ...
    function _assignPoints(
        ...
        //Extract user address from Hookdata Note: we cannot extract user address by msg.sender becoz msg.sender is v4 router
